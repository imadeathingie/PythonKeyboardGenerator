class Keyboard:
    ...
    def intersections(self, key, keys):
        ...
        intersections = []
        if 't' in neighs:
            str_t =self.translate(neighs['t'], self.hull(self.br(neighs['t'])[2:] + self.bl(neighs['t'])[2:]))
            str =self.translate(key, self.hull(self.tr(key)[2:] + self.tl(key)[2:]))
            intersections.append(f"hull(){{\n  {str_t}\n  {str}\n  }}\n")
            if 'r' in neighs:
                str_t =self.translate(neighs['t'], self.hull(self.br(neighs['t'])[2:]))
                str =self.translate(key, self.hull(self.tr(key)[2:]))
                str_r =self.translate(neighs['r'], self.hull(self.tl(neighs['r'])[2:]))
                intersections.append(f"hull(){{\n  {str_t}\n  {str_r}\n  {str}\n  }}\n")
            if 'l' in neighs:
                str_t =self.translate(neighs['t'], self.hull(self.bl(neighs['t'])[2:]))
                str =self.translate(key, self.hull(self.tl(key)[2:]))
                str_l =self.translate(neighs['l'], self.hull(self.tr(neighs['l'])[2:]))
                intersections.append(f"hull(){{\n  {str_t}\n  {str_l}\n  {str}\n  }}\n")
            if 'tr' in diags:
                str_t =self.translate(neighs['t'], self.hull(self.br(neighs['t'])[2:]))
                str =self.translate(key, self.hull(self.tr(key)[2:]))
                str_tr =self.translate(diags['tr'], self.hull(self.bl(diags['tr'])[2:]))
                intersections.append(f"hull(){{\n  {str_t}\n  {str_tr}\n  {str}\n  }}\n")
            if 'tl' in diags:
                str_t =self.translate(neighs['t'], self.hull(self.bl(neighs['t'])[2:]))
                str =self.translate(key, self.hull(self.tl(key)[2:]))
                str_tl =self.translate(diags['tl'], self.hull(self.br(diags['tl'])[2:]))
                intersections.append(f"hull(){{\n  {str_t}\n  {str_tl}\n  {str}\n  }}\n")
        
        if 'r' in neighs:
            str_r =self.translate(neighs['r'], self.hull(self.tl(neighs['r'])[2:] + self.bl(neighs['r'])[2:]))
            str =self.translate(key, self.hull(self.tr(key)[2:] + self.br(key)[2:]))
            intersections.append(f"hull(){{\n  {str_r}\n  {str}\n  }}\n")
